...
3. Create scope files for each team at `.project-orchestrator/scopes/{team}.json`.
4. Make the script executable: `chmod +x scripts/scope-protection.sh`

## Scope File Format

Create `.project-orchestrator/scopes/{team}.json` where `{team}` matches your implementation team name (e.g., `implement-my-feature`):
...
- Reads the scope file for the current team from `.project-orchestrator/state.json`
